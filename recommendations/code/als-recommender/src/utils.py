...
def get_movieId(movies_df, fav_movie_list, movie_to_idx):
    """
...
def add_new_user_to_data(train_df, movieId_list, ratings_df):
...
def recommend(model, train_df, movie_list, movies_df, ratings_df, mapper, pretrained=False, k=3):
    
    train_df = train_df.iloc[:, :3].copy()
    train_df.columns = ['user', 'item', 'rating']
    
    movieId_list = get_movieId(movies_df, movie_list, mapper)
    train_df = add_new_user_to_data(train_df, movieId_list, ratings_df)
